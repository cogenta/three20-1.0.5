...
- (void)updateChrome {
  ...
  playButton.enabled = _photoSource.nPhotos > 1;
  _previousButton.enabled = _centerPhotoIndex > 0;
  _nextButton.enabled = _centerPhotoIndex >= 0 && _centerPhotoIndex < _photoSource.nPhotos-1;
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)slideshowTimer {
  if (_centerPhotoIndex == _photoSource.nPhotos-1) {
...
- (void)moveToNextValidPhoto {
  if (_centerPhotoIndex >= _photoSource.nPhotos) {
    // We were positioned at an index that is past the end, so move to the last photo
    [self moveToPhotoAtIndex:_photoSource.nPhotos - 1 withDelay:NO];
